        ...
        <div key={answer.$id} className="flex gap-4">
          <div className="flex shrink-0 flex-col items-center gap-4">
            <VoteButtons
              type="answer"
              id={answer.$id}
              upvotes={answer.upvotesDocuments ?? { documents: [], total: 0 }}
              downvotes={answer.downvotesDocuments ?? { documents: [], total: 0 }}
            />

            {user?.$id === answer.authorId && (
              <button
                className="flex h-10 w-10 items-center justify-center rounded-full border border-red-500 p-1 text-red-500 hover:bg-red-500/10"
